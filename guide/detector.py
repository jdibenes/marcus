...
def get_angle(A: Tuple[int, int], B: Tuple[int, int], C: Tuple[int, int]) -> float:
    ...
    try:
        ...
        #raise e
        return math.pi if (a==0 and b==0) else math.pi/2
...
class CustomContour(object):
    def __init__(
        self, rubiks_parent, index: int, contour: ndarray, heirarchy: ndarray, debug: bool
    ) -> None:
...
class RubiksOpenCV(object):
    ...
    def analyze(self, image, cube_size: None = None) -> bool:
        MIN_AREA = 100
        ...
        #nonoise = cv2.fastNlMeansDenoising(gray, h=15, templateWindowSize=7, searchWindowSize=21)
        nonoise = cv2.fastNlMeansDenoising(gray, h=15, templateWindowSize=7, searchWindowSize=11)
        ...
        for component in zip(contours, hierarchy):
            ...
            if con.area > MIN_AREA and con.cX is not None:
        ...
        raw_data = []
        self._sorted_candidates = self.sort_by_row_col(deepcopy(self.candidates), self.size)
        for con in self._sorted_candidates:
        ...
        self._sorted_colors = raw_data
        if (len(self._sorted_candidates) != 9 or len(self._sorted_colors) != 9):
            return False
    ...
    def get_centers_and_colors(self):
        centers = [tuple([con.cX, con.cY]) for con in self._sorted_candidates]
        colors = [tuple([rgb[2], rgb[1], rgb[0]]) for rgb in self._sorted_colors]
